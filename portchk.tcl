...
proc portchk_scan_pub {nick uhost hand chan text} {
	...
	if {$port == ""} {
		...
		if {[catch {set sock [socket -async $host $port]} error]} {
			putquick "PRIVMSG $chan :\00315,1\[\0037,1Check\00315,1\] \0030,1Connection to $host \($port\) was refused."
		} else {
...
proc portchk_connected_pub {chan sock host port timerid} {
	...
	if {$error != ""} {
		close $sock
		putquick "PRIVMSG $chan :\00315,1\[\0037,1Check\00315,1\] \0030,1Connection to $host \($port\) failed. \([string totitle $error]\)"
	} else {
		fileevent $sock writable {}
		fileevent $sock readable [list portchk_read_pub $chan $sock $host $port]
		putquick "PRIVMSG $chan :\00315,1\[\0037,1Check\00315,1\] \0030,1Connection to $host \($port\) accepted."
	}
}
proc portchk_timeout_pub {chan sock host port} {
	close $sock
	putquick "PRIVMSG $chan :\00315,1\[\0037,1Check\00315,1\] \0030,1Connection to $host \($port\) timed out."
}
